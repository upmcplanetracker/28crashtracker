...
- [28CrashTracker](#28crashtracker)
  ...
  - [Configuration (.env file)](#configuration-env-file)
    ...
    - [Required for Both Platforms](#required-for-both-platforms)
    - [Bluesky Credentials (if POSTING\_PLATFORM=bluesky)](#bluesky-credentials-if-posting_platformbluesky)
    - [X.com API Credentials (if POSTING\_PLATFORM=x)](#xcom-api-credentials-if-posting_platformx)
    - [Optional](#optional)
  - [API Services and Costs](#api-services-and-costs)
    - [1. RapidAPI (Waze Alerts)](#1-rapidapi-waze-alerts)
    - [2. OpenCage Geocoding (Optional, but Recommended Failover)](#2-opencage-geocoding-optional-but-recommended-failover)
    - [Bluesky](#bluesky)
...
2. **Install Python Dependencies:**
   ...
   - **Note:** The `crash.py` script uses `requests`, `json`, `os`, `random`, `datetime`, `atproto`, `geopy`, `dotenv`, `zoneinfo`, `time`, `logging`, and `sys`. You will need to create a `requirements.txt` file containing these dependencies if it's not already present in your repository. You can generate it by running `pip freeze > requirements.txt` after manually installing them, or just list them directly:
     ...
     python-dotenv
     ```
     _Self-correction: Based on the `crash.py` content, `json`, `os`, `random`, `datetime`, `zoneinfo`, `time`, `logging`, `sys` are standard library modules and do not need to be listed in `requirements.txt`._
...
2. **Add necessary variables:** Populate the `.env` file with the variables described in the [User Controllable Settings](#user-controllable-settings) section, along with your API keys.
   ...
   ```bash
   POSTING_PLATFORM=bluesky  # or 'x' for X.com
   ```
   ...
   ### Bluesky Credentials (if POSTING_PLATFORM=bluesky)
   ...
   ### X.com API Credentials (if POSTING_PLATFORM=x)
...
2. **API Key:**
   ...
   _Note: If `OPENCAGE_API_KEY` is not provided in your `.env` file, the script will log a warning, and OpenCage geocoding will not be available as a failover._
...
3. Generate access tokens for your app
...
2. **Add a Cron Job:**
   Add a line to your crontab file to execute the script every 10 minutes. This frequency should help you stay under the 10,000 API calls per month limit (10,000 calls / (30 days _ 24 hours _ 6 calls/hour) $\approx$ 5.7 minutes per call, so 10 minutes is a safe interval).
...
- **`DUPLICATE_TIME_MINUTES`** (Internal constant in `crash.py`): The maximum time difference in minutes for two incidents to be considered duplicates.
  ...
  - **What it does:** Works with `DUPLICATE_DISTANCE_KM` to define what constitutes a duplicate incident. An incident is a duplicate if it's within this time window _and_ within the distance threshold of a previously seen incident.
