...
def load_last_prompts(file_path):
    ...
    try:
        ...
        logging.info(f"'{file_path}' not found or empty. Starting with no recent prompts.")
        return []
    except IOError as e:
    ...
    for attempt in range(MAX_WAZE_API_RETRIES + 1):
        ...
        try:
            ...
            if attempt < MAX_WAZE_API_RETRIES:
                ...
                logging.error(f"Failed to fetch Waze data after {MAX_API_RETRIES + 1} attempts. Final error: {e}")
                return []
...
def post_to_bluesky(text, embed_url=None, embed_title=None, embed_description=None, local_image_path=None, max_retries=3):
    """
        ...
        try:
            ...
            if local_image_path:
                try:
                    ...
                    logging.info(f"Uploading local image/GIF from {local_image_path} to Bluesky...")
...
def load_monthly_crash_data():
    ...
    try:
        ...
        logging.info(f"'{MONTHLY_CRASH_FILE}' not found. Initializing and creating with default data.")
        ...
        try:
            ...
            logging.info(f"'{MONTHLY_CRASH_FILE}' successfully created with initial data.")
        except IOError as e:
            logging.critical(f"Critical: Failed to create and save '{MONTHLY_CRASH_FILE}': {e}. Proceeding with in-memory default data.")
        return default_data
    except json.JSONDecodeError:
        logging.error(f"Error decoding JSON from '{MONTHLY_CRASH_FILE}'. Re-initializing and saving default data.")
        default_data = {"current_month_crashes": 0, "last_reset_date": datetime.now().strftime("%Y-%m-%d")}
        try:
            with open(MONTHLY_CRASH_FILE, 'w') as f:
                json.dump(default_data, f, indent=2)
            logging.info(f"'{MONTHLY_CRASH_FILE}' overwritten with default data due to corruption.")
        except IOError as e:
            logging.critical(f"Critical: Failed to save '{MONTHLY_CRASH_FILE}' after decode error: {e}. Proceeding with in-memory default data.")
        return default_data
    except IOError as e:
        logging.critical(f"Permission error or other I/O error reading '{MONTHLY_CRASH_FILE}': {e}. Cannot load monthly crash data. Proceeding with in-memory default data.")
...
def increment_monthly_counter():
    ...
    logging.info(f"Monthly crash counter incremented. Current count: {data['current_month_crashes']}")
...
def handle_monthly_reset_and_report():
    ...
    if is_end_of_month_and_time or is_start_of_new_month:
        ...
        # MODIFIED CALL TO post_to_bluesky for GIF embed
        ...
        if post_successful:
            logging.info(f"Monthly crash report for {reported_month_name} posted successfully.")
        ...
        logging.info("Not the end of the month for reporting/resetting or already processed for this month.")
            ...
            if alert.get("type") == "ACCIDENT":
                ...
                if (re.search(r'\b(ROUTE|RT|US|STATE ROUTE|PA)[ -]?28\b', street, re.IGNORECASE) or \
                   ...
                   ("BUS" not in street.upper()):
                    ...
                    chosen_prompt, post_text, Maps_url, embed_desc = format_alert(alert, recent_prompts)
                    logging.info(f"Attempting to post new alert {alert_id} to Bluesky with prompt: '{chosen_prompt}'.")

                    # Original call to post_to_bluesky for daily reports (NO GIF)
                    ...
                    if post_successful:
                        ...
                        recent_prompts = add_prompt_to_history(chosen_prompt, recent_prompts)
